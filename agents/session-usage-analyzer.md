...
Custom to this kit. **Install at user scope** (`~/.claude/agents/`) — it reads global `~/.claude/projects/` transcripts, `~/.claude/plugins/`, and global agents/skills, and composes with user-scope tools. Data feed: the official `session-report` plugin's `analyze-sessions.mjs`. Doctrine lineage: `project-optimizer` Check 1 (session-cost audit) and the shanraisshan/anthropic context-budget thresholds. Designed to compose with — not duplicate — `session-report`, `project-optimizer`, and `/save`. Reviewed by `subagent-design-reviewer` over two passes; revisions applied: task unit defined, output contract fixed, boundaries encoded as body rules, single-session targeting + JSON validation added, install scope stated.
