...
Run all five in order. Each check produces findings tagged `pass` / `watch` / `flag`. For every flag, the report must include: (a) the file or setting, (b) the value found, (c) the threshold or rule violated, (d) the concrete remedy. **Phrase every remedy as an enforced change, not advice** — passive guidance gets ignored the same way an unenforced rule in a doc does. Cut a tool from an agent's grant rather than telling it not to use the tool; add a `paths:` scope rather than asking the model to ignore a rule; convert a thrice-repeated correction into a `hookify` block rather than just noting the pattern. The value is the enforceable delta, not the observation.
...
3. **Primitive-to-task fit.** For each skill, subagent, command, hook, and plugin in `~/.claude/` and `.claude/`, compare against the decision tree. Flag misfits — a subagent doing work a skill could handle in-session, a skill that should have been a subagent for isolation, a multi-primitive workflow scattered across separate files instead of bundled in a plugin. Also evaluate composition — see "Workflow composition patterns" below. **Role-taxonomy lens:** classify each subagent by function and flag tool grants that contradict the role — an *orchestrator* (coordinates/delegates) holding `Edit`/`Write`/`MultiEdit`; a *planner* (analyzes/specifies) with vague, non-actionable output or broad write tools it never uses; a *doer* (implements) with unrestricted `*` access or that sub-delegates instead of doing the work. A subagent can be the right primitive and still the wrong shape; the flag is the tool/role mismatch, remedied by cutting the offending tool.
