...
| `doc-sync-analyzer` | Read-only analyzer invoked by the `doc-sync` skill: given a session's git diff + transcript, maps changed code to the project's docs (CLAUDE.md, README, CHANGELOG, `docs/`, API refs, examples) and reports which are stale, with paste-ready fix diffs. Proposes, never edits — every staleness claim quotes the doc line *and* the contradicting code line (zero-hallucination discipline). Distinct from `technical-writer` (writes docs from scratch) and `project-optimizer` (audits doc structure/size). | Read, Glob, Grep, Bash | Custom |
...
| `mcp-developer` | Build/debug/optimize MCP servers and clients — JSON-RPC 2.0, resource/tool/prompt defs, transports, SDK (TS/Python), auth. Defaults to a Claude Code host. Bounded against `prompt-engineer` (owns server, not host prompts). | Read, Write, Edit, Bash, Glob, Grep | Adapted from [VoltAgent/awesome-claude-code-subagents](https://github.com/VoltAgent/awesome-claude-code-subagents) (`categories/06-developer-experience/mcp-developer.md`, commit `6f804f0`), hardened — removed context-manager opener + JSON handshake + fabricated metrics + cross-agent roster; collapsed inventory |
| `cli-developer` | Design/improve command-line tools — subcommand structure, flags, config layering, prompts, completions, exit codes, cross-platform. Fit for kit tooling, UE5 automation wrappers, portfolio CLIs. Bounded against `cpp-pro`/`game-developer`. | Read, Write, Edit, Bash, Glob, Grep | Adapted from [VoltAgent/awesome-claude-code-subagents](https://github.com/VoltAgent/awesome-claude-code-subagents) (`categories/06-developer-experience/cli-developer.md`, commit `6f804f0`), hardened — removed handshake + fabricated metrics + cross-agent roster + product-team framing; collapsed inventory |
| `first-principles-thinking` | Read-only reasoning agent — strips a problem to irreducible truths and rebuilds. Surfaces/challenges assumptions for design, strategy, architecture decisions. Bounded against `research-scout`/`research-analyst` (evidence) and `5-whys`/`debugger` (single-failure root cause). | Read, Grep, Glob, WebFetch, WebSearch | Adapted from [VoltAgent/awesome-claude-code-subagents](https://github.com/VoltAgent/awesome-claude-code-subagents) (`categories/10-research-analysis/first-principles-thinking.md`, commit `6f804f0`), hardened — removed cross-agent section; added read-only/advisory framing + boundaries + `model` |
| `api-documenter` | OpenAPI 3.1 specs, endpoint/schema/error docs, multi-language code examples. Complements `technical-writer` (prose). Fit for UE5 HTTP/service boundaries and Claude tool/MCP defs. **Write scoped to doc/spec artifacts, never source.** | Read, Write, Edit, Glob, Grep, WebFetch, WebSearch | Adapted from [VoltAgent/awesome-claude-code-subagents](https://github.com/VoltAgent/awesome-claude-code-subagents) (`categories/07-specialized-domains/api-documenter.md`, commit `6f804f0`), hardened — removed handshake + fabricated metrics + cross-agent roster; **added Write-scope constraint**; collapsed inventory |
...
| `save` | Persist session context to both supabrain (`capture_thought` MCP for cross-session/cross-project memory) and local project memory files. Routes per-item based on cross-project utility; defaults to both when uncertain. Final step invokes `session-archive` (best-effort) to bank the session into the ClaudeOptimizer corpus. **Note:** distinct from the `/save` slash command in `commands/` (which runs a session-grading rubric); same name, different surface. | Custom |
| `session-archive` | Bank the current session into the [ClaudeOptimizer](https://github.com/mbschenked/ClaudeOptimizer) corpus: a metrics-stamped markdown card (exact numbers from `extract-metrics.mjs`/`build-card.mjs` — never hand-typed) + a verbatim raw-transcript copy + a per-project `LEDGER.md` row. Resolves the corpus root via `$CLAUDE_OPTIMIZER_ROOT` → OS default; **skips gracefully when the repo isn't on this machine** so it never blocks `/save`. Invoke via `/session-archive`, "archive this session," or as `/save`'s last step. | Custom |
...
| `doc-sync` | Reactive project-docs updater. Forks to the `doc-sync-analyzer` subagent, injects the session's `git diff` (committed + staged) for orientation, locates the session transcript for intent, then triages *all* project docs (CLAUDE.md, README, CHANGELOG, `docs/`, API refs, examples) for staleness. Returns a 6-section brief with confidence tags + paste-ready fix diffs; proposes, never edits. Invoke via `/doc-sync` or "what docs are stale after this session." Fills the ecosystem gap no Anthropic/VoltAgent/shanraisshan artifact covered (their nearest is manual + CLAUDE.md-only). | Custom |
