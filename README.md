...
| `ai-writing-auditor` | Audit text for AI-writing tells (formatting, sentence-structure, tiered vocabulary) by severity and return a humanized rewrite. Resumes, portfolio copy, posts, docs. Preserves author voice; never fabricates facts. **Currently gated locally — observed to overshoot (avoids AI tells but produces phrasings a person wouldn't say). Use with caution.** | Read, Write, Edit, Glob, Grep | Adapted from [VoltAgent/awesome-claude-code-subagents](https://github.com/VoltAgent/awesome-claude-code-subagents) (`categories/04-quality-security/ai-writing-auditor.md`), hardened — stripped cross-agent section + Bash; tightened triggers. Upstream credit: `conorbronsdon/avoid-ai-writing` (MIT) |
| `code-reviewer` | Review code against project guidelines in CLAUDE.md (style, bugs, quality) with high-precision confidence scoring to minimize false positives. Single-perspective by default; the `/code-reviewer` fan-out command launches multiple `code-review-worker` instances in parallel for substantive reviews — that's the default for "review this code." | Read, Grep, Glob | Vendored from Anthropic pr-review-toolkit |
| `code-review-worker` | Fan-out worker for the `/code-reviewer` orchestrator. Axis-blind single-perspective review with ≥40 confidence floor (orchestrator gates). Standalone use only for trivial one-file changes or reviewer benchmarks; default is the fan-out command. Pairs with `code-reviewer`. | Read, Grep, Glob | Vendored from Anthropic pr-review-toolkit, customized for fan-out architecture |
...
| `debugger` | Debugging specialist for errors, test failures, and unexpected behavior. Layers failing-test-first + one-variable-at-a-time + 3-strikes hard stop on top of the Anthropic canonical. **Sonnet validation pending** — Opus 4.7 dogfood on Unity C# passed; pair with `code-reviewer` (Pattern A) for patches heading to source; not autonomous on production bugs. | Read, Edit, Bash, Grep, Glob | Anthropic canonical subagent + 3 systematic-debugging disciplines |
