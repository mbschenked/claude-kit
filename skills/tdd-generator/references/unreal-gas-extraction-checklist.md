...
Load this when the target is an Unreal project (a `*.uproject` exists). It is the field guide for Step 1 of `tdd-generator` — what to look for, where, and which grep anchors find it. Extract evidence (file paths + types), not prose. Skip rows the project doesn't use; never invent a row to look complete.
